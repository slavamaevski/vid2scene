...
def generate_planar_projections_from_equirectangular(
    ...
    for i in crop_factor:
        if i < 0 or i > 1:
            logger.error("Invalid crop factor. All values must be in [0,1].")
            raise ValueError("Invalid crop factor. All values must be in [0,1].")
    ...
    for i in tqdm.tqdm(os.listdir(frame_dir), total=num_ims, file=sys.stdout):
...
def compute_resolution_from_equirect(image_dir: str, num_images: int) -> Tuple[int, int]:
...
def process_equirectangular_images(
    image_dir: str,
    output_dir: str,
    samples_per_im: int = 8,
    crop_top: float = 0.0,
    crop_bottom: float = 0.0,
    crop_left: float = 0.0,
    crop_right: float = 0.0,
) -> str:
    """Process equirectangular images and convert them to planar projections.
    
    Args:
        image_dir: Directory containing equirectangular images
        samples_per_im: Number of samples per image (8 or 14)
        output_dir: Optional output directory (defaults to image_dir/planar_projections)
        crop_top: Crop factor for top (0.0-1.0)
        crop_bottom: Crop factor for bottom (0.0-1.0)
        crop_left: Crop factor for left (0.0-1.0)
        crop_right: Crop factor for right (0.0-1.0)
        
    Returns:
        Path to the directory containing the planar projections
    """
    # Validate crop factors
    for crop_value in [crop_top, crop_bottom, crop_left, crop_right]:
        if crop_value < 0.0 or crop_value > 1.0:
            logger.error("Crop factors must be between 0.0 and 1.0")
            raise ValueError("Crop factors must be between 0.0 and 1.0")
    
    crop_factor = (crop_top, crop_bottom, crop_left, crop_right)
    
    # Compute resolution based on number of samples
    logger.info(f"Computing resolution for {samples_per_im} samples per image")
    planar_image_size = compute_resolution_from_equirect(image_dir, samples_per_im)
    logger.info(f"Using planar image size: {planar_image_size}")
    
    # Generate planar projections
    logger.info(f"Generating planar projections from equirectangular images in {image_dir}")
    output_path = generate_planar_projections_from_equirectangular(
        image_dir=image_dir,
        output_dir=output_dir,
        planar_image_size=planar_image_size,
        samples_per_im=samples_per_im,
        crop_factor=crop_factor
    )
    
    logger.info(f"Planar projections saved to {output_path}")
    return output_path


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate planar projections from equirectangular images.")
    parser.add_argument("image_dir", type=str, help="Directory containing equirectangular images")
    parser.add_argument("--output_dir", type=str, help="Output directory for planar projections", required=True)
    parser.add_argument("--samples_per_im", type=int, default=8, choices=[8, 14], help="Number of samples per image (8 or 14)")
    parser.add_argument("--crop_top", type=float, default=0.0, help="Crop factor for top (0.0-1.0)")
    parser.add_argument("--crop_bottom", type=float, default=0.0, help="Crop factor for bottom (0.0-1.0)")
    parser.add_argument("--crop_left", type=float, default=0.0, help="Crop factor for left (0.0-1.0)")
    parser.add_argument("--crop_right", type=float, default=0.0, help="Crop factor for right (0.0-1.0)")
    ...
    try:
        process_equirectangular_images(
            image_dir=image_dir,
            samples_per_im=args.samples_per_im,
            output_dir=output_dir,
        ...
        logger.error(f"Error processing equirectangular images: {e}")
        sys.exit(1)